...
export type EmailAuthStore = {
  ...
  actions: {
    ...
    togglePasswordVisibility: (field: 'password' | 'confirmPassword' | 'recoveryCode') => void;
    setErrorMessage: (error: string) => void;
    changeCurrentIndex: () => void;
    completeInitialRender: () => void;
    manageTimer: (stepId: number, action: 'start' | 'tick' | 'clear', duration?: number) => void;
    resetForm: () => void;
  };
};
  ...
  actions: {
    next: () => {
      ...
      if (nextIndex < pages - 1) {
        console.log('useEmailAuthStore: next called, moving to nextIndex=', nextIndex + 1);
        set({
          ...
          form: {
            ...
            confirmPassword: nextIndex === 2 ? '' : get().form.confirmPassword, 
            recoveryCode: nextIndex === 5 ? '' : get().form.recoveryCode, 
          },
        });
      } else if (nextIndex === pages - 1) {
        
        console.log('useEmailAuthStore: next called, moving from step 6 to step 4');
        set({
          nextIndex: 3, 
          errorMessage: '',
          form: {
    ...
    handleForgotPassword: () => {
      console.log('useEmailAuthStore: handleForgotPassword called, moving to step 5');
      set({
        nextIndex: 4, 
        errorMessage: '',
        form: {
    ...
    changeCurrentIndex: () =>
      set((state) => {
        console.log('useEmailAuthStore: changeCurrentIndex, setting currentIndex=', state.nextIndex);
      ...
      if (action === 'start' && duration) {
        ...
        set((state) => {
          ...
          return {
            timers: {
              ...state.timers,
              [stepId]: { seconds: timer.seconds - 1, canResend: false },
            },
          };
        });
